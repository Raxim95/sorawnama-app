...
let [T1, T2] = read("/material/t.tex").split("++++").map(v => new PFSHA(v.split("\n")));
let [A1, A2, A3, B1, B2, B3, C1, C2, C3] = read("/material/rus.tex").split("++++").map(v => new PFSHA(v.split("\n====\n")));
// ------------------------------------------------
let variant = "";
let VARIANT_SANI = 100;

for (let i = 0; i < VARIANT_SANI; i++) {
	let [t1, t2] = [T1.next(), T2.next()]
...
// ------------------------------------------------
fs.writeFileSync(__dirname + `/sizalg rus 31-may.tex`, texShablon(variant));
