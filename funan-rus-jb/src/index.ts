...
let tex = read("/../material/" + til + ".tex");

let All = _.compact(tex.split("++++"));
...
for (let i = 0; i < VARIANT_SANI; i++) {
	...
	variant += shablon(i, S, til.toUpperCase());
}
// ------------------------------------------------
fs.writeFileSync(__dirname + `/funan-${til}-jb.tex`, texShablon(variant));
