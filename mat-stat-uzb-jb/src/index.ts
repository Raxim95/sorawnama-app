import * as fs from "fs";
import * as _ from "lodash";
import PFSHA from "./PFSHA";
import shablon from "./shablon";
import texShablon from "./tex-shablon";

const read = (path: string) => fs.readFileSync(__dirname + path, "utf8").replaceAll("\r\n", "\n");

let qq = read("/../material/uz.tex");
qq.replaceAll("\r\n", "\n")
	.replaceAll("\n\n", "\n")
	.replaceAll("\\textbf{++++}", "++++")
	.replaceAll("====\n++++", "++++")
	.replaceAll("====++++", "++++")
	.replaceAll("++++\n====", "++++")
	.replaceAll("++++====", "++++")
	.replaceAll("\n\n", "\n");

let All = _.compact(qq.split("++++"));
let All_Quesions = All.map((T) => _.compact(T.split("====")));

const noShuffles = [2, 3, 4, 5, 8];

let All_PFSHA = All_Quesions.map((T, i) => {
	if (noShuffles.includes(i)) return new PFSHA(T, false);
	else return new PFSHA(T);
});

// ------------------------------------------------
let variant = "";
let VARIANT_SANI = 100;

for (let i = 0; i < VARIANT_SANI; i++) {
	let S = All_PFSHA.map((T) => T.next());
	variant += shablon(i, S, "");
}
// ------------------------------------------------
fs.writeFileSync(__dirname + `/matstat-uzb-jb.tex`, texShablon(variant));
